import numpy as np
import matplotlib.pyplot as plt
import os
import json
import cv2
import sys
import random
import argparse
try:
    import gurobipy as gp
    from gurobipy import GRB
    GUROBI_AVAILABLE = True
except ImportError:
    GUROBI_AVAILABLE = False
    print("Warning: Gurobi not available. Use --solver cbc, glpk, or scip for open-source alternatives.")

from scipy import ndimage
from ilp_solver import get_solver, list_available_solvers
from benchmark import SolverBenchmark
from skimage import io
from skimage.transform import rescale, resize, downscale_local_mean
from tqdm import tqdm
import torch.multiprocessing as mp
import torch
import torchvision.transforms as T
from torch.utils.data import Dataset, DataLoader
from sklearn.mixture import GaussianMixture
import monai
import pickle as pkl
import torch.nn.functional as F
import torch.nn as nn
import shutil
from monai.networks import one_hot
from segment_anything import SamPredictor, sam_model_registry
from segment_anything.utils.transforms import ResizeLongestSide
from torch.utils.data.sampler import SubsetRandomSampler
from datetime import datetime
import time
import gc
from sklearn.cluster import KMeans


# best hyperparam by validation set : tnbc (2,6,25,5). monuseg(2,6,25,5). consep(2,5,20,5)

parser = argparse.ArgumentParser()
parser.add_argument("--img_dir_path", type=str)
parser.add_argument("--box_dir_path", type=str)
parser.add_argument("--sam_s_path", type=str, default = None) # needed for sam-ilp
parser.add_argument("--gt_dir_path", type=str, default = None) # if you want to score the predictions as well.
parser.add_argument("--save_path", type=str)
parser.add_argument("--mu", type=float, default=2)
parser.add_argument("--nc", type=int, default=6)
parser.add_argument("--sigma", type=int, default=25)
parser.add_argument("--split", type=int, default=5)
parser.add_argument("--model_weights", type=str)
parser.add_argument("--model_type", type=str, choices = ['vit_b', 'vit_l', 'vit_h'], default='vit_b')
parser.add_argument("--gurobi_license", type=str, default = None)
parser.add_argument("--gurobi_license_file", type=str, default = None)
parser.add_argument("--solver", type=str, 
                    choices=['gurobi', 'cbc', 'glpk', 'scip', 'ortools', 'graphcut', 'alpha_expansion', 'sparse'], 
                    default='cbc', 
                    help="ILP solver to use. Exact: gurobi, cbc, glpk, scip. Approximate: ortools, graphcut, alpha_expansion, sparse")
parser.add_argument("--solver_time_limit", type=int, default=60, 
                    help="Time limit for ILP solver in seconds")
parser.add_argument("--list_solvers", action='store_true', 
                    help="List available solvers and exit")
parser.add_argument("--benchmark", action='store_true',
                    help="Run performance benchmark on all available solvers")
parser.add_argument("--benchmark_sizes", type=int, nargs='+', default=[50, 100, 200, 300],
                    help="Image sizes for scalability benchmark")
parser.add_argument("--accuracy_latency_analysis", action='store_true',
                    help="Run accuracy-latency trade-off analysis")
parser.add_argument("--sparse_threshold", type=float, default=0.1,
                    help="Sparsity threshold for sparse solver")
parser.add_argument("--downsample_factor", type=int, default=2,
                    help="Downsample factor for sparse solver")
parser.add_argument("--alpha_iterations", type=int, default=10,
                    help="Max iterations for alpha-expansion solver")
parser.add_argument("--mode", type=str, choices = ['d-sam', 'sam-ilp'], default = 'sam-ilp')
parser.add_argument("--type", type=str, choices = ['instance', 'semantic'], default = 'semantic')
args = parser.parse_args()

# List available solvers if requested
if args.list_solvers:
    print("Available ILP solvers:")
    available = list_available_solvers()
    print("\nExact solvers:")
    for solver in ['gurobi', 'cbc', 'glpk', 'scip']:
        if solver in available:
            status = "✓" if available[solver] else "✗"
            print(f"  {status} {solver}")
    
    print("\nApproximate solvers:")
    for solver in ['ortools', 'graphcut', 'alpha_expansion', 'sparse']:
        if solver in available:
            status = "✓" if available[solver] else "✗"
            print(f"  {status} {solver}")
    
    print(f"\nTotal available: {sum(available.values())}/{len(available)}")
    exit(0)

# Validate solver choice
available_solvers = list_available_solvers()
if args.solver not in available_solvers or not available_solvers[args.solver]:
    print(f"Error: Solver '{args.solver}' not available.")
    print("Available solvers:")
    for solver, is_available in available_solvers.items():
        if is_available:
            print(f"  ✓ {solver}")
    exit(1)

# if args.mode == 'sam-ilp':
#     env = gp.Env()
#     os.environ['GRB_LICENSE_FILE']=args.gurobi_license_file
#     env.setParam('LicenseKey', args.gurobi_license)

device = 'cuda' if torch.cuda.is_available() else 'cpu'

def show_mask(mask, ax, random_color=False):
    if random_color:
        color = np.concatenate([np.random.random(3), np.array([0.6])], axis=0)
    else:
        color = np.array([251/255, 252/255, 30/255, 0.6])
    h, w = mask.shape[-2:]
    mask_image = mask.reshape(h, w, 1) * color.reshape(1, 1, -1)
    ax.imshow(mask_image)

def show_point(points, ax):
    for point in points:
        ax.plot(point[0], point[1], '-ro')

def logit(x):
    a = torch.tensor(x)
    return torch.special.logit(a, eps=1e-7)

def track_mem():
    t = torch.cuda.get_device_properties(0).total_memory
    r = torch.cuda.memory_reserved(0)
    a = torch.cuda.memory_allocated(0)
    f = r-a  # free inside reserved
    print(f't={t/1e9}, r={r/1e9}, a={a/1e9}, f={f/1e9}')

def free_mem():
    torch.cuda.empty_cache()
    gc.collect()
    
def compute_dice_coefficient(mask_gt, mask_pred):
    volume_sum = mask_gt.sum() + mask_pred.sum()
    if volume_sum == 0:
        return np.NaN
    volume_intersect = (mask_gt & mask_pred).sum()
    return 2*volume_intersect / volume_sum

def largest_connected_component(binary_mask):
    labeled_mask, num_features = ndimage.label(binary_mask)
    fin = ndimage.sum(binary_mask, labeled_mask, range(1, num_features + 1))
    fin = np.argsort(fin)
    if len(fin) == 0:
        return binary_mask
    return (labeled_mask == fin[-1]+1) * 1

def eval_image(sam_model, image, file, mode):
    # Start timing for entire image processing
    image_start_time = time.time()
    
    free_mem()
    mask_save_path = args.save_path
    os.makedirs(mask_save_path, exist_ok = True)
    
    img = cv2.imread(image)
    # img = cv2.resize(img, (100, 100))
    predictor = SamPredictor(sam_model)
    predictor.set_image(img)
    img = img.astype(int)
    
    weak_supervision = np.loadtxt(file, delimiter = ',', dtype = int)

    gt_mask = None
    if args.gt_dir_path != None:
        gt_mask = np.load(os.path.join(args.gt_dir_path, image.split('/')[-1][:-4]), allow_pickle = True)
    
    d_sam_pred = np.zeros(gt_mask.shape)
        
    entity = 2
    for enu, i in enumerate(weak_supervision):
        box = i[3:]
        masks, conf, raw_logits = predictor.predict(box = box, multimask_output = False)

        if args.type == 'semantic':
            d_sam_pred += masks[0,:,:]
        elif args.type == 'instance':
            to_assign = masks[0,:,:] * entity
            d_sam_pred = np.where((to_assign > 0), to_assign, d_sam_pred)
        entity += 1
    
    if mode == 'd-sam':
        if args.type == 'semantic':
            cv2.imwrite(os.path.join(args.save_path, image.split('/')[-1].split('\\')[-1]), (d_sam_pred > 0) * 255)
        elif args.type == 'instance':
            cv2.imwrite(os.path.join(args.save_path, image.split('/')[-1].split('\\')[-1]), d_sam_pred)
        
        # Report total image processing time for d-sam mode
        total_image_time = time.time() - image_start_time
        print(f"Total image processing time (d-sam): {total_image_time:.4f} seconds")
        return
        
    elif mode == 'sam-ilp':

        sam_s_pred = cv2.imread(os.path.join(args.sam_s_path, image.split('/')[-1].split('\\')[-1]))
        
        if not np.all(sam_s_pred.shape == d_sam_pred.shape):
            sam_s_pred = (cv2.resize(sam_s_pred.astype(float), d_sam_pred.shape) > 0) * 1
            sam_s_pred = sam_s_pred[:,:,0]
        
        all_fg = img[(d_sam_pred != 0) & (sam_s_pred != 0)]
        all_bg = img[(d_sam_pred == 0) & (sam_s_pred == 0)]

        final_llh = np.zeros(gt_mask.shape)
        
        
        ln = img.shape[0]
        bt = img.shape[1]
        
        split = args.split
        
        # Timing for tile processing
        total_tile_time = 0
        total_tiles = args.split * args.split
        
        for i in range(args.split):
            for j in range(args.split):
                tile_start_time = time.time()
                
                pc = img[j*ln//split:j*ln//split+ln//split, i*bt//split:i*bt//split+bt//split]
                
                foreground_pixels = pc[(d_sam_pred[j*ln//split:j*ln//split+ln//split, i*bt//split:i*bt//split+bt//split] != 0) & (sam_s_pred[j*ln//split:j*ln//split+ln//split, i*bt//split:i*bt//split+bt//split] != 0)]
                
                background_pixels = pc[(d_sam_pred[j*ln//split:j*ln//split+ln//split, i*bt//split:i*bt//split+bt//split] == 0) & (sam_s_pred[j*ln//split:j*ln//split+ln//split, i*bt//split:i*bt//split+bt//split] == 0)]
                
                if len(foreground_pixels) <= args.nc:
                    foreground_pixels = all_fg
    
                if len(background_pixels) <= args.nc and len(foreground_pixels) > args.nc:
                    background_pixels = all_bg
                    
                fg_gmm = GaussianMixture(n_components=args.nc, random_state=0)
                fg_gmm.fit(foreground_pixels)
                bg_gmm = GaussianMixture(n_components=args.nc, random_state=0)
                bg_gmm.fit(background_pixels)
    
                llh_fg = fg_gmm.score_samples(pc.reshape((-1,pc.shape[2]))).reshape((ln//split, bt//split))
                llh_bg = bg_gmm.score_samples(pc.reshape((-1,pc.shape[2]))).reshape((ln//split, bt//split))
                
                llh_fg = np.clip(llh_fg, -100,100)
                llh_bg = np.clip(llh_bg, -100,100)
                llh_fg = np.exp(llh_fg)
                llh_bg = np.exp(llh_bg)
            
                llh = llh_fg/(llh_bg + llh_fg)
            
                final_llh[j*ln//split:j*ln//split+ln//split, i*bt//split:i*bt//split+bt//split] = llh
                
                # Record tile processing time
                tile_time = time.time() - tile_start_time
                total_tile_time += tile_time
    
                # idx = np.s_[j*ln//args.split:j*ln//args.split+ln//args.split, i*bt//args.split:i*bt//args.split+bt//args.split]
                # pc = img[idx]
                # print(pc.shape)
                # foreground_pixels = pc[(d_sam_pred[idx] != 0) & (sam_s_pred[idx] != 0)]
                # background_pixels = pc[(d_sam_pred[idx] == 0) & (sam_s_pred[idx] == 0)]

                # if len(foreground_pixels) <= args.nc:
                #     foreground_pixels = pc[(sam_s_pred[idx] != 0)]
                    
                # if len(foreground_pixels) <= args.nc:
                #     foreground_pixels = pc[(d_sam_pred[idx] != 0)]

                # if len(background_pixels) <= args.nc:
                #     background_pixels = all_bg

                # fg_gmm = GaussianMixture(n_components=args.nc, random_state=0)
                # fg_gmm.fit(foreground_pixels)
                # bg_gmm = GaussianMixture(n_components=args.nc, random_state=0)
                # bg_gmm.fit(background_pixels)
                        
                # llh_fg = fg_gmm.score_samples(pc.reshape((-1,pc.shape[2]))).reshape((ln//args.split, bt//args.split))
                # llh_bg = bg_gmm.score_samples(pc.reshape((-1,pc.shape[2]))).reshape((ln//args.split, bt//args.split))
                
                # llh_fg = np.clip(llh_fg, -100,100)
                # llh_bg = np.clip(llh_bg, -100,100)
                # llh_fg = np.exp(llh_fg)
                # llh_bg = np.exp(llh_bg)
            
                # llh = llh_fg / (llh_bg + llh_fg)
                # final_llh[idx] = llh

        # Report average tile processing time
        avg_tile_time = total_tile_time / total_tiles
        print(f"Average tile processing time: {avg_tile_time:.4f} seconds")

        d_sam_component = (d_sam_pred > 0) * 1
        start_time = time.time()
        
        # Initialize the ILP solver
        solver_kwargs = {
            'time_limit': args.solver_time_limit,
            'verbose': True  # Can be made configurable
        }
        
        # Add solver-specific parameters
        if args.solver == 'gurobi':
            solver_kwargs.update({
                'license_key': args.gurobi_license,
                'license_file': args.gurobi_license_file
            })
        elif args.solver == 'sparse':
            solver_kwargs.update({
                'sparsity_threshold': args.sparse_threshold,
                'downsample_factor': args.downsample_factor
            })
        elif args.solver == 'alpha_expansion':
            solver_kwargs['max_iterations'] = args.alpha_iterations
        
        try:
            solver = get_solver(args.solver, **solver_kwargs)
            if args.solver == 'sparse':
                print(f"Sparse solver config: threshold={args.sparse_threshold}, downsample={args.downsample_factor}")
            sam_ilp_pred = solver.solve_sam_ilp(img, sam_s_pred, d_sam_component, final_llh, args.mu, args.sigma)
        except Exception as e:
            print(f"Error with {args.solver} solver: {e}")
            if args.solver == 'sparse':
                print("Note: Sparse solver has been recently fixed for OpenCV compatibility.")
                print("If issues persist, try: --sparse_threshold 0.2 --downsample_factor 4")
            print("Falling back to CBC solver...")
            solver = get_solver('cbc', time_limit=args.solver_time_limit, verbose=True)
            sam_ilp_pred = solver.solve_sam_ilp(img, sam_s_pred, d_sam_component, final_llh, args.mu, args.sigma)
        
        ilp_time = time.time() - start_time
        print(f"ILP solver time: {ilp_time:.4f} seconds")

        if args.type == 'semantic':
            cv2.imwrite(os.path.join(args.save_path, image.split('/')[-1].split('\\')[-1]), (sam_ilp_pred > 0) * 255)
            # Report total image processing time
            total_image_time = time.time() - image_start_time
            print(f"Total image processing time: {total_image_time:.4f} seconds")
            
        elif args.type == 'instance':
            sam_ilp_pred = np.where((sam_ilp_pred > 0) & (d_sam_pred > 0), d_sam_pred, sam_ilp_pred)
            # print(sam_ilp_pred)
            dsam_in_ilp = sam_ilp_pred.copy()
            mask2 = np.where((sam_ilp_pred > 0) & (d_sam_pred > 0), 0, sam_ilp_pred)
        
            curr_cells = np.unique(sam_ilp_pred)[-1]
            sem_map, num_sem_map = ndimage.label((mask2+sam_ilp_pred) > 0)
            
            for c in range(1,num_sem_map+1):
                pts = np.unique(sam_ilp_pred[(sem_map == c)])
                # print(pts)
                if len(pts) > 1 and pts[0] == 1:
                    num_cells = max(1, len(pts)-1)
                    # print(num_cells, num_sem_map, c)
                    kmeans = KMeans(n_clusters=num_cells, init=np.array([[j.mean() for j in np.nonzero(sam_ilp_pred == i)] for i in pts[1:]]), n_init=1)
                    x_ones, y_ones = np.nonzero((sem_map == c))
                    # print(x_ones, y_ones)
                    kmeans.fit([[x_ones[i], y_ones[i]] for i in range(len(x_ones))])
                    x_ones, y_ones = np.nonzero((sam_ilp_pred == 1) & (sem_map == c))
                    # print(x_ones, y_ones)
                    labels = [int(pts[i+1]) for i in kmeans.predict([[x_ones[i], y_ones[i]] for i in range(len(x_ones))])]
                    # print(labels)
                    sam_ilp_pred[x_ones, y_ones] = labels
                elif pts[0] == 1 and len(pts) == 1:
                    curr_cells += 1
                    sam_ilp_pred[(sem_map == c)] = curr_cells
            cv2.imwrite(os.path.join(args.save_path, image.split('/')[-1].split('\\')[-1]), sam_ilp_pred)

        # Report total image processing time
        total_image_time = time.time() - image_start_time
        print(f"Total image processing time: {total_image_time:.4f} seconds")

        # Clear memory after each image processing
        del sam_ilp_pred, d_sam_component, final_llh, sam_s_pred, d_sam_pred, img, gt_mask
        gc.collect()
        free_mem()
        
        return
        
def eval_images(sam_model, list_of_images, mode):
    dices = {}
    total_processing_time = 0
    
    for i, image in tqdm(enumerate(list_of_images)):
        print(f"\n--- Processing Image {i+1}/{len(list_of_images)}: {image.split('/')[-1]} ---")
        
        # Time individual image processing
        start_time = time.time()
        eval_image(sam_model, image, os.path.join(args.box_dir_path, list_of_images[i].split('.')[0].split('/')[-1]+'.txt'), mode)
        image_time = time.time() - start_time
        total_processing_time += image_time
        
        print(f"Image {i+1} completed in: {image_time:.4f} seconds")
        print(f"Average time per image so far: {total_processing_time/(i+1):.4f} seconds")

    print(f"\n=== FINAL STATISTICS ===")
    print(f"Total images processed: {len(list_of_images)}")
    print(f"Total processing time: {total_processing_time:.4f} seconds")
    print(f"Average time per image: {total_processing_time/len(list_of_images):.4f} seconds")

    # img_dice = sum(dices.values())
    # return dices, img_dice / len(dices)

sam_model = sam_model_registry[args.model_type](checkpoint=args.model_weights).to(device)

list_of_images = os.listdir(args.img_dir_path)
list_of_images = [os.path.join(args.img_dir_path, i) for i in list_of_images]

# Benchmarking mode
if args.benchmark or args.accuracy_latency_analysis:
    print("Running benchmark mode...")
    
    # Initialize benchmark system
    benchmark = SolverBenchmark(output_dir=os.path.join(args.save_path, "benchmark_results"))
    
    # Get available solvers for benchmarking
    available = list_available_solvers()
    benchmark_solvers = {}
    
    for solver_name in ['cbc', 'ortools', 'graphcut', 'alpha_expansion', 'sparse']:
        if available.get(solver_name, False):
            solver_kwargs = {
                'time_limit': args.solver_time_limit,
                'verbose': False
            }
            
            # Add solver-specific parameters
            if solver_name == 'sparse':
                solver_kwargs.update({
                    'sparsity_threshold': args.sparse_threshold,
                    'downsample_factor': args.downsample_factor
                })
            elif solver_name == 'alpha_expansion':
                solver_kwargs['max_iterations'] = args.alpha_iterations
            
            try:
                benchmark_solvers[solver_name] = get_solver(solver_name, **solver_kwargs)
            except Exception as e:
                print(f"Failed to initialize {solver_name}: {e}")
    
    # Include Gurobi if available
    if available.get('gurobi', False):
        try:
            gurobi_kwargs = {
                'time_limit': args.solver_time_limit,
                'verbose': False,
                'license_key': args.gurobi_license,
                'license_file': args.gurobi_license_file
            }
            benchmark_solvers['gurobi'] = get_solver('gurobi', **gurobi_kwargs)
        except Exception as e:
            print(f"Failed to initialize Gurobi: {e}")
    
    print(f"Benchmarking {len(benchmark_solvers)} solvers: {list(benchmark_solvers.keys())}")
    
    # Prepare test data (use first few images)
    test_images = list_of_images[:min(3, len(list_of_images))]  # Limit for benchmarking
    
    if args.benchmark:
        print("Running scalability benchmark...")
        
        # Use first image for scalability test
        if test_images:
            first_image = test_images[0]
            img = cv2.imread(first_image)
            
            # Load corresponding data
            box_file = os.path.join(args.box_dir_path, first_image.split('/')[-1].split('.')[0] + '.txt')
            if os.path.exists(box_file):
                weak_supervision = np.loadtxt(box_file, delimiter=',', dtype=int)
                
                # Generate sam_s_pred and d_sam_pred (simplified for benchmark)
                predictor = SamPredictor(sam_model)
                predictor.set_image(img)
                
                d_sam_pred = np.zeros(img.shape[:2])
                for i in weak_supervision:
                    box = i[3:]
                    masks, _, _ = predictor.predict(box=box, multimask_output=False)
                    d_sam_pred += masks[0, :, :]
                d_sam_pred = (d_sam_pred > 0).astype(float)
                
                # Mock sam_s_pred and likelihood
                sam_s_pred = (np.random.rand(*img.shape[:2]) > 0.3).astype(float)
                llh = np.random.rand(*img.shape[:2])
                
                # Load ground truth if available
                gt = None
                if args.gt_dir_path:
                    gt_path = os.path.join(args.gt_dir_path, first_image.split('/')[-1][:-4])
                    if os.path.exists(gt_path):
                        gt = np.load(gt_path, allow_pickle=True)
                
                # Run scalability test
                scalability_results = benchmark.run_scalability_test(
                    benchmark_solvers, img, sam_s_pred, d_sam_pred, llh, gt,
                    mu=args.mu, sigma=args.sigma, sizes=args.benchmark_sizes
                )
                
                # Generate report
                report = benchmark.generate_report()
                print("\nBenchmark completed! Results saved to:", benchmark.output_dir)
                print("Report preview:")
                print(report[:1000] + "..." if len(report) > 1000 else report)
    
    if args.accuracy_latency_analysis:
        print("Running accuracy-latency analysis...")
        
        # Prepare test cases
        test_cases = []
        for img_path in test_images[:2]:  # Limit for analysis
            img = cv2.imread(img_path)
            img = cv2.resize(img, (200, 200))  # Standard size for analysis
            
            # Mock data for analysis
            sam_s_pred = (np.random.rand(200, 200) > 0.3).astype(float)
            d_sam_pred = (np.random.rand(200, 200) > 0.4).astype(float)
            llh = np.random.rand(200, 200)
            
            test_cases.append({
                'img': img,
                'sam_s_pred': sam_s_pred,
                'd_sam_pred': d_sam_pred,
                'llh': llh,
                'gt': None  # Would load actual GT in real scenario
            })
        
        # Run analysis
        mu_values = [0.5, 1.0, 2.0] if args.accuracy_latency_analysis else [args.mu]
        sigma_values = [10, 25, 50] if args.accuracy_latency_analysis else [args.sigma]
        
        analysis_results = benchmark.run_accuracy_latency_analysis(
            benchmark_solvers, test_cases, mu_values, sigma_values
        )
        
        # Generate analysis report
        report = benchmark.generate_report()
        print("\nAccuracy-latency analysis completed!")
        print("Results saved to:", benchmark.output_dir)
    
    exit(0)

# Regular processing mode
# per_img_score, average_score = eval_images(sam_model, list_of_images, args.mode)
eval_images(sam_model, list_of_images, args.mode)